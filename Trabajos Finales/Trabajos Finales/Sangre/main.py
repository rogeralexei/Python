...
def guardarcambios(lista, archivotxt):
    if archivotxt == 'donante' or archivotxt == 'donatario':
...
def modificar(lista, donante_donatario):
    ...
    if cliente:
        ...
        if will == 1:
            ...
            nuevaedad = ask('Ingrese la nueva Edad: ',int,range(6,121),False,'\nAqui debes poner un numero entre 6 y 120... vamos de nuevo\n')
        ...
        if donatario:
            ...
            for donante in donantes:
                if donante['Sangre'] in sangre_permitida:
                    ...
                    print('\nLo sentimos, no tenemos un donante para su tipo de sangre...')
                    posibletransf = False
            ...
            return print(f'\nLo sentimos, el Donatario {nombredonatario} {apellidodonatario} no se encuentra registrado.\n')
...
def buscar(lista, donante_donatario, nombre, apellido):
    ...
    if cliente_ == {}:
        return False    
    elif nombre != cliente_['Nombre'] and apellido != cliente_['Apellido']:
        return False
    else:
        return cliente_
...
def estadisticas(lista,tipo, donante_donatario):
    ...
    if tipo == 'Sangre':
        ...
        tipoe = []
        nedades = []
        adolescentes = range(12,19)
        juventud = range(20,27)
        adultez = range(27,60)
        vejez = range(60,121)
        for paciente in lista:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if will == 1:
            ...
            will = ask('\nCual estadistica desea ver??: ',int,range(1,3),False,'\nAqui debes poner un numero entre 1 y 2, vamos de nuevo...\n')
            if will == 1:
                print('1.) Donantes \n2.) Donatarios')
                will = ask('\nCual lista desea ver??: ',int,range(1,3),False,'\nAqui debes poner un numero entre 1 y 2, vamos de nuevo...\n')
                if will == 1:
            ...
            print('\nMuchas Gracias por acudir a nosotros, que tenga un buen dia!')
            break
